...
const SectionPage = ({ page, routePageId = null, onLinkClick, isEditMode, isLiveMode = false, indexPageOrdinal = null, onCellChange, onHeadingChange, onImageChange, onIndexChange, onImageClick, allIndexItems, allPages = [] }) => {
  ...
  if (page.pageType === 'text-only') {
    ...
    return (
      <div style={{ lineHeight: 1.6, fontFamily }}>
        ...
        <div style={{ whiteSpace: 'pre-wrap', fontSize: `${contentFontSize}rem`, color: contentTextColor }}>{page.content}</div>
      </div>
    );
  }
  ...
  if (page.pageType === 'image-text') {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: page.imagePosition === 'left' ? '1fr 1fr' : '1fr 1fr', gap: '2rem', alignItems: 'center' }}>
        ...
        <div style={{ color: contentTextColor, fontSize: `${contentFontSize}rem`, lineHeight: 1.6, fontFamily }}>
          <p>{page.content}</p>
  ...
  if (page.pageType === 'managed-content' || page.pageType === 'meanaged-content') {
    ...
    // Read-only: TipTap — render saved HTML
    if (currentTemplate === 'tiptap-editor') {
      return (
        <div>
          {page.title && (
            <div style={{
          ...
          <div
            className="managed-content-display tiptap-readonly"
            style={{ fontFamily, fontSize: `${contentFontSize}rem`, color: contentTextColor, lineHeight: 1.6 }}
            dangerouslySetInnerHTML={{ __html: page.tiptapHtml || '' }}
          />
        </div>
      );
    }
    ...
    <div style={{ fontFamily }}>
      ...
      {page.content && (
        Array.isArray(page.content) ? (
          <ul style={{ color: contentTextColor, fontSize: `${contentFontSize}rem`, lineHeight: 1.6 }}>
            {page.content.map((item, idx) => (
              <li key={idx}>{item}</li>
            ))}
          </ul>
        ) : (
          <div style={{ color: contentTextColor, fontSize: `${contentFontSize}rem`, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
            {String(page.content || '')}
          </div>
        )
      )}
