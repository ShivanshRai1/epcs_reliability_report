...
const PublishSelectionModal = ({ 
  ...
  const displayPageNumber = (page) => {
    const numericPageNumber = Number(page?.pageNumber ?? page?.page_number);
    if (Number.isFinite(numericPageNumber) && numericPageNumber > 0) {
      return Number.isInteger(numericPageNumber)
        ? numericPageNumber
        : Math.ceil(numericPageNumber);
    }

    const sortedPages = (reportData?.pages || [])
      .filter((item) => !item?._isDraftDeleted)
      .sort((a, b) => (a.pageNumber || 0) - (b.pageNumber || 0));
    const position = sortedPages.findIndex((item) => String(item.id) === String(page?.id));
    ...
    <Modal isOpen={isOpen} onClose={onCancel}>
      <div className="publish-selection-modal">
        ...
        <div className="changes-list">
              ...
              <div className="change-items">
                {editedPagesList.map(page => (
                  <label key={page.id} className="change-item">
                    ...
                    />
                    <span className="page-title">Page {displayPageNumber(page)} ({page.pageType || page.pageTemplate || 'Content'}) - {page.title || 'Untitled Page'}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {/* New Pages - use getPageNumber for accurate sequential number */}
