...
const PublishSelectionModal = ({ 
  ...
  const nonDeletedSorted = (reportData?.pages || [])
    .filter(p => !p._isDraftDeleted)
    .sort((a, b) => (a.pageNumber || 0) - (b.pageNumber || 0));

  const computedPageNumbers = new Map();
  nonDeletedSorted.forEach((p, i) => computedPageNumbers.set(String(p.id), i + 1));

  const getPageNumber = (pageId) => {
    // Primary: look up by ID in the computed map
    const byId = computedPageNumbers.get(String(pageId));
    if (byId) return byId;
    // Fallback: find page by ID in reportData and use its sort position
    const page = (reportData?.pages || []).find(p => String(p.id) === String(pageId));
    if (!page) return '?';
    const pos = nonDeletedSorted.findIndex(p => String(p.id) === String(pageId));
    return pos >= 0 ? pos + 1 : Math.round(page.pageNumber) || '?';
  };
